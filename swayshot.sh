...
if type wl-copy >/dev/null  2>&1; then
	printf "%s" "$SCREENSHOT_FULLNAME" | wl-copy
elif type xsel >/dev/null  2>&1; then
	printf "%s" "$SCREENSHOT_FULLNAME" | xsel --clipboard
elif type xclip >/dev/null 2>&1; then
	printf "%s" "$SCREENSHOT_FULLNAME" | xclip -selection clipboard
else
	printf "%s" "$SCREENSHOT_FULLNAME"
fi
